...
async function safeWrite(congress: number, type: BillType, number: number) {
    try {
        await writeBillToSQL(pool, congress, type, number);
        console.log(`wrote ${congress}-${type}-${number}`);
    } catch (e) {
        if (e == 'reserved') console.log(`reserved ${congress}-${type}-${number}`);
        else if (e == 'ratelimit') console.log('ratelimited! reset API key');
        else if (e == 'exists') console.log(`exists ${congress}-${type}-${number}`);
        else {}//throw e;
    }
}
