import type {
    ...
    BillCosponsorsResponse, MemberResponse,
    Paginated,
    PaginationReference, VoteMembersResponse
} from "./queryTypes.js";
import type {Bill, BillType, Date, Representative, State, Vote} from "./sqlTypes.js";
...
export const ACTION_CODE_MAP: {[c: string]: State|null} = {
...
function fillAPI(api: string, params: any) {
    return api.replace(/{(.*?)}/g, (_, key) => params[key])
}

async function query<T>(api: string, params: any) {
...
async function queryPaginated<T extends Paginated>(data: PaginationReference, paginateUpon: string) {
    ...
    while (start < count) {
        ...
        const res = JSON.parse(f) as T;//(await (f).json()) as T;

        if (!result) result = res; // @ts-ignore
        else result[paginateUpon] = result[paginateUpon].concat(res![paginateUpon]);
...
export async function scrapeBill(congress: number, type: BillType, number: number): Promise<BillResponse> {
    const bill = (await query<BillBaseResponse>(BILL_BASE, {congress, billType: type, billNumber: number})).bill;
    if (!bill) throw "ratelimit";
    if (!bill.actions) throw "reserved";
    const actions = (await queryPaginated<BillActionsResponse>(bill.actions, "actions"))!;
    const committees = bill.committees && (await queryPaginated<BillCommitteesResponse>(bill.committees, "committees"));
    const cosponsors = bill.cosponsors && await queryPaginated<BillCosponsorsResponse>(bill.cosponsors, "cosponsors");

    let houseVote = undefined;
    let senateVote = undefined;

    let lastTransition: Date|undefined = undefined;
    ...
    for (const action of actions.actions) {
        if (action.sourceSystem?.code != 9) continue; // LoC events only
        ...
        if (state !== null) {
            if (stateHistory.length > 0) {
                if (state == 'law' && stateHistory[0] == 'law') continue; // skip duplicate
        ...
        if (action.recordedVotes) {
            for (const vote of action.recordedVotes) {
                if (vote.chamber == "House") houseVote ??= generateVoteUID(vote);
                if (vote.chamber == "Senate") senateVote ??= generateVoteUID(vote);
            }
        }
    }
...
export async function scrapeSenateVote(uid: string): Promise<SenatorVoteResponse> {
    const { congress, session, rollNumber } = parseVoteUID(uid);

    const json = new XMLParser().parse(await (await fetch(fillAPI(SENATE_VOTE,
        {congress, session, rollNumber: rollNumber.toString().padStart(5, '0') }))).text())['roll_call_vote'];
    const date = new Date(json['vote_date']);

    const votes = json['members']['member'];

    const yeas = await Promise.all(votes.filter((v:any)=>v['vote_cast'] == 'Yea').map(getBioguide));
    const nays = await Promise.all(votes.filter((v:any)=>v['vote_cast'] == 'Nay').map(getBioguide));
    return {
        vote: {
            uid,
            yeas,
        ...
        repRefs: await Promise.all(votes.map(async (v:any) => {return {
...
export async function scrapeHouseVote(uid: string): Promise<HouseVoteResponse> {
    ...
    return {
        vote: {
            uid,
            yeas: votes.filter((v:any)=>v.vote == 'Yea' || v.vote == 'Aye').map((v:any)=>v.legislator['@_name-id']),
            nays: votes.filter((v:any)=>v.vote == 'Nay' || v.vote == 'No').map((v:any)=>v.legislator['@_name-id']),
        ...
        repRefs: votes.map((v:any)=>v.legislator['@_name-id']),
    }
}

export async function scrapeHouseRep(bioguideId: string): Promise<Representative> {
    const {member} = await query<MemberResponse>(MEMBER_BASE, {bioguideId});
