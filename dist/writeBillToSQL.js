...
async function insertRepRefs(pool, repRefs) {
    for (const rep of repRefs) {
        if (await hasRep(pool, rep.uid))
            continue;
        try {
...
let reps = [];
async function hasRep(pool, uid) {
    if (reps.length == 0)
        reps = (await pool.query("SELECT uid FROM reps")).rows.map(r => r.uid);
    return reps.includes(uid);
}
export async function writeBillToSQL(pool, congress, type, number) {
    if ((await pool.query("SELECT uid FROM bills WHERE uid=$1", [generateBillUID(congress, type, number)])).rowCount)
        throw 'exists';
...
//# sourceMappingURL=writeBillToSQL.js.map